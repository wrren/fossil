...
class Model {
	...
	static attachRelationships( model, obj ) {
		...
		r.belongsTo.forEach( ( belongs, index, array ) => {
			obj[namer.toMemberName(belongs.name)] = function() {
				return belongs.find( obj[namer.toForeignKeyID(belongs.name)] );
			}
		} );

		r.hasMany.forEach( ( has, index, array ) => {
			obj[namer.plural(namer.toMemberName(has.name))] = function() {
		    ...
		    .then( ( fields ) => {
			    return q.then( ( rows ) => {
				    return rows.map( ( row, index, array ) => {
					    return Model.toObject( row, model, fields );
				    } );
			    } );
		    } );
	}
	...
	//
	static find( query ) {
		return Model._query( 	db( namer.toTableName( this.name ) ).select( '*' ).where( query ).limit( 1 ),
		    			this )
			.then( ( models ) => {
	...
	static where( query, options ) {
		let select 	= db( namer.toTableName( this.name ) ).select( '*' ).where( query );
