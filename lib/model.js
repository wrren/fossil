...
var db         		= null;
var dbUtils 		= null;
var tableReader		= null;
var modelFields		= new Map();
...
class Model {
	...
	//
	static find( query ) {
		return this.fields( namer.toTableName( this.name ) )
		    .then( ( fields ) => {
	...
	static _query( q, model ) {
		...
		return this.fields( namer.toTableName( model.name ) )
		    .then( ( fields ) =>
			q.then( ( rows ) =>
	...
	//		that resolves to a ModelFields object for the specified table
	//
	static fields( table ) {
		if( tableReader == null ) {
			tableReader = dbUtils.tables()
				.then( ( f ) => {
					modelFields = f;
					return modelFields;
				} );
		} else if( modelFields != null ) {
			tableReader = Promise.resolve( modelFields );
		}
		return tableReader.then( ( f ) => {
			if( table == undefined ) {
				return f;
			} else {
				return f.get( table );
